...
export default React.memo(function MovieCard({
  ...
  const getStatusIcon = (status: Status) => {
    switch (status) {
      case "WATCHED":
        return (
          <span
            className="material-symbols-outlined text-tertiary rounded-full"
            style={{ fontVariationSettings: "'FILL' 1" }}
          >
            check_circle
          </span>
      ...
      case "TO_WATCH":
        return (
          <span
            className="material-symbols-outlined text-secondary rounded-full"
            style={{ fontVariationSettings: "'FILL' 1" }}
          >
            schedule
          </span>
  ...
  const cycleStatus = (e: React.MouseEvent) => {
    ...
    e.stopPropagation();
    const next =
      !movie.status || movie.status === "UNWATCHED"
        ? "TO_WATCH"
  ...
  const borderColor =
    movie.status === "WATCHED"
      ? "border-tertiary shadow-[0_0_20px_rgba(40,167,69,0.15)] ring-1 ring-tertiary/50"
      : movie.status === "TO_WATCH"
        ? "border-secondary shadow-[0_0_20px_rgba(0,176,215,0.15)] ring-1 ring-secondary/50"
        : "border-white/5 hover:border-primary/50";

  const glowClass = isPhaseComplete
    ? "shadow-[0_0_30px_rgba(255,215,0,0.3)] border-yellow-500/50"
    : "";
    ...
    >
      <div className="aspect-[2/3] w-full overflow-hidden rounded-xl mb-3 md:mb-4 relative bg-surface-container-highest isolate shadow-inner">
        ...
        {(!posterUrl || !imageLoaded) && (
          <div
            className={`w-full h-full bg-white/5 animate-pulse flex flex-col items-center justify-center p-4 md:p-6 bg-gradient-to-b from-surface-container-highest to-background ${posterUrl ? "absolute inset-0" : ""}`}
          >
            <div className="w-10 h-10 md:w-16 md:h-16 rounded-xl md:rounded-2xl bg-white/10 flex items-center justify-center mb-4 md:mb-6 shadow-inner">
              <span className="material-symbols-outlined text-white/20 text-xl md:text-3xl">
                image
              </span>
            </div>
        ...
        >
          {movie.status === "WATCHED" ? (
            <Check className="w-3.5 h-3.5 md:w-4 md:h-4 text-tertiary drop-shadow-[0_0_8px_rgba(40,167,69,0.6)]" />
          ) : movie.status === "TO_WATCH" ? (
            <Plus className="w-3.5 h-3.5 md:w-4 md:h-4 text-secondary drop-shadow-[0_0_8px_rgba(0,176,215,0.6)]" />
          ) : (
        ...
        <div className="hidden md:flex absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex-col items-center justify-center backdrop-blur-sm gap-2 pointer-events-none z-20">
          <span className="material-symbols-outlined text-primary text-6xl font-light scale-90 group-hover:scale-110 transition-transform duration-500 drop-shadow-[0_0_15px_#E62429]">
            play_circle
          </span>
          <span className="text-white text-[10px] font-black uppercase tracking-[0.3em] bg-primary/20 px-4 py-1.5 rounded-full border border-primary/30 shadow-[0_0_20px_rgba(230,36,41,0.2)]">
            Classified Data
          </span>
        </div>
      </div>
      ...
      <div className="flex flex-col gap-1.5 px-0.5 pb-1">
        <div className="flex items-center justify-between gap-2 overflow-hidden">
          <span className="text-[8px] md:text-[9px] font-mono uppercase tracking-[0.2em] text-primary font-black truncate">
            {movie.phase}
          </span>
          <div className="flex items-center gap-1 shrink-0">
            <span className="material-symbols-outlined text-[10px] md:text-[12px] text-zinc-500">
              schedule
            </span>
            <span className="text-[9px] md:text-[10px] font-mono text-zinc-400 font-bold">
              {movie.runtime}m
            </span>
          </div>
        </div>

        <h4 className="text-[11px] md:text-[13px] font-black text-white/90 leading-tight uppercase tracking-tight group-hover:text-primary transition-colors duration-300 line-clamp-2 md:line-clamp-1 h-[2.2em] md:h-auto">
          {movie.title}
        </h4>

        <div className="flex items-center gap-1.5 mt-0.5">
          <div
            className={`w-5 h-5 md:w-7 md:h-7 rounded-lg border flex items-center justify-center transition-all ${
              movie.status === "WATCHED"
          ...
          <div className="flex items-center gap-1">
            {[...Array(movie.postCredits)].map((_, i) => (
              <div
                key={i}
                className="w-1 h-1 md:w-1.5 md:h-1.5 rounded-full bg-primary/40 shadow-[0_0_5px_rgba(230,36,41,0.5)]"
              />
            ))}
          </div>
        </div>
...
function getHeroIcon(movie: any) {
  ...
  if (t.includes("guardians") || t.includes("star-lord"))
