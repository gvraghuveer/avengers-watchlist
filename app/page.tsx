      ...
      <div className="sticky top-16 md:top-20 z-30 mb-12 -mx-4 md:-mx-12 px-4 md:px-12 py-4 md:py-6 bg-background/80 backdrop-blur-3xl border-b border-white/5 shadow-2x">
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6 md:gap-8">
          <div className="flex flex-col gap-1 w-full lg:w-auto">
            <h3 className="text-2xl md:text-4xl font-light tracking-tight text-on-surface">Avengers Archive</h3>
            ...
            <div className="flex flex-col sm:flex-row sm:items-center gap-4 mt-4">
              ...
              <div className="flex flex-wrap gap-2">
                {[
                  ...
                  <div key={filter.label} className="relative group/filter flex-1 sm:flex-none min-w-[140px]">
                    <select 
                      value={filter.value}
          ...
          <div className="flex items-center gap-4 md:gap-6 w-full lg:w-auto overflow-x-auto no-scrollbar pb-2 lg:pb-0">
            ...
            <div className="flex items-center gap-4 md:gap-6 bg-white/5 border border-white/5 px-4 md:px-6 py-3 rounded-2xl md:rounded-[2rem] shadow-inner shrink-0">
              <div className="text-right">
                <p className="text-[8px] md:text-[9px] font-mono tracking-[0.2em] uppercase text-zinc-500 mb-0.5">Left</p>
                <p className="text-lg md:text-xl font-bold text-white tracking-tighter leading-none">{(remainingRuntime/60).toFixed(1)}<span className="text-[10px] text-zinc-500 ml-1">H</span></p>
              </div>
              <div className="w-px h-6 md:h-8 bg-white/10" />
              <div className="text-right">
                <p className="text-[8px] md:text-[9px] font-mono tracking-[0.2em] uppercase text-zinc-500 mb-0.5">Done</p>
                <p className="text-lg md:text-xl font-bold text-tertiary tracking-tighter leading-none">{(watchedRuntime/60).toFixed(1)}<span className="text-[10px] text-zinc-500 ml-1">H</span></p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-3 md:gap-5 bg-white/5 border border-white/10 px-4 md:px-5 py-3 rounded-2xl md:rounded-[2rem] shadow-2xl backdrop-blur-md shrink-0">
              <div className="text-right">
                <p className="text-[8px] md:text-[9px] font-mono tracking-[0.2em] uppercase text-secondary mb-0.5">Progress</p>
                <div className="text-xl md:text-2xl font-black text-white tracking-tighter leading-none">
                  ...
                  <span className="text-white/20 mx-1.5 md:mx-2 text-lg md:text-xl">/</span> 
              ...
              <div className="w-10 h-10 md:w-12 md:h-12 rounded-full border-2 border-surface-container flex items-center justify-center relative bg-surface-container-highest">
                <svg className="absolute inset-0 w-full h-full -rotate-90">
                  <circle cx="20 md:22" cy="20 md:22" r="18 md:20" className="stroke-white/5" strokeWidth="4" fill="none" />
                  <circle cx="20 md:22" cy="20 md:22" r="18 md:20" className="stroke-tertiary drop-shadow-[0_0_5px_rgba(40,167,69,0.8)] transition-all duration-1000 ease-out" strokeWidth="4" fill="none" strokeDasharray="125.6" strokeDashoffset={125.6 - (125.6 * (archiveCount / movies.length))} strokeLinecap="round" />
                </svg>
                <span className="material-symbols-outlined text-[14px] md:text-[16px] text-white/50">data_exploration</span>
              </div>
            </div>
          </div>
        </div>
